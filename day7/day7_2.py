...
class IntCode:
    ...
    def run_op(self):
        ...
        if action is ops.SUM:     # 1 Sum
            ...
            #print("INPUT")
            #self.ins[self.ins[self.index+1]] = int(self.inputs[self.i_n%2])
...
if __name__ == "__main__":
    ...
    for comb in itertools.permutations([5,6,7,8,9]):
        computers = [
            IntCode(numbers.copy(),[comb[0], 0]),
            IntCode(numbers.copy(),[comb[1]]),
            IntCode(numbers.copy(),[comb[2]]),
            IntCode(numbers.copy(),[comb[3]]),
            IntCode(numbers.copy(),[comb[4]])
        ]
        foutput = 0
        i = 0
        while not computers[-1].finished:
            computers[i%5].output = None 
            output = computers[i%5].compute_solution()
            #output = 0
            #print(f"Computer{i%5} ({computers[i%5].inputs}) -> {output}")
            computers[(i+1)%5].inputs.append(output)    
            i += 1
            if output: foutput = output

        print("Final -> ", foutput)

        if foutput > max_out: max_out = foutput
        
    print("Final max -> ", max_out)
